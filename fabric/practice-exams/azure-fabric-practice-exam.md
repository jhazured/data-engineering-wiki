...
### 3. Power BI Desktop project (.pbip)
...
### 4. Discovering assets across the organization
...
### 6. Exposing a saved query in a warehouse
...
### 7. Star schema in lakehouse or warehouse
...
### 8. Querying a KQL database or eventhouse
...
### 9. Real-time data and filter pushdown
...
### 13. Direct Lake fallback to DirectQuery
...
### 18. External data in OneLake without copying
...
### 20. Endorsement (Promoted/Certified)
...
### 24. Lakehouse file-level permissions
...
### 25. Version control for a workspace
...
### 26. Impact analysis of downstream dependencies
...
### 27. Deploy and manage semantic models via XMLA
...
### 28. Power BI data source (.pbids) file
...
### 29. Data connection and on-premises gateway
...
### 31. OneLake integration for eventhouse and semantic models
...
### 36. Duplicate data, missing data, null values
...
### 37. Converting column data types
...
### 38. Filtering data in transformation
...
### 40. DAX for select, filter, aggregate
...
### 41. Star schema in a semantic model
...
### 42. Bridge table for many-to-many
...
### 47. Large semantic model storage format
...
### 48. Performance improvements in queries and report visuals
...
### 50. Direct Lake on OneLake vs Direct Lake on SQL endpoint
...
### 51. Power BI template (.pbit) file
...
### 52–75. Explanations (batch)

**52.** **C) Contributor** – Contributor can create and delete items but cannot share the workspace or manage access; that is an Admin capability.  
**53.** **B)** – Build permission allows creating reports and content that use the model; Edit/Admin may be needed to change the model.  
**54.** **B)** – RLS is defined in the semantic model via roles and DAX filter expressions.  
**55.** **C)** – Object-level security can restrict access to specific tables, columns, or measures.  
**56.** **B)** – Sensitivity labels apply to workspaces and supported items (reports, semantic models, dataflows, etc.).  
**57.** **B)** – Certified usually requires formal review and indicates higher trust; Promoted is a lighter endorsement.  
**58.** **B)** – Git-connected workspaces version item definitions in source-control-friendly formats.  
**59.** **B)** – Moving content between stages is deploying or promoting.  
**60.** **B)** – Shared semantic models let multiple reports and users connect to one published model.  
**61.** **B)** – Dataflows and pipelines can connect to many sources (Azure SQL, SharePoint, REST, Fabric items, etc.).  
**62.** **B)** – OneLake data hub is for discovering and browsing endorsed/shared assets.  
**63.** **B)** – Warehouse provides a relational T-SQL experience (tables, views, stored procedures).  
**64.** **B)** – Lakehouse combines structured/semi-structured data with Delta/Parquet and SQL/Spark.  
**65.** **B)** – Stored procedures run T-SQL batches for ETL, SCD2, or other logic.  
**66.** **B)** – Adding columns (e.g., combined name) is enriching data.  
**67.** **B)** – Dimension tables hold descriptive attributes and keys.  
**68.** **B)** – Remove duplicates or group-by to resolve duplicates.  
**69.** **B)** – Warehouse and lakehouse (via SQL endpoint) support ad-hoc SQL.  
**70.** **B)** – KQL is for event/log/time-series data in eventhouses and KQL databases.  
**71.** **B)** – Import mode copies data into the model; refresh updates it.  
**72.** **B)** – Many-to-many can use a bridge table or built-in M2M.  
**73.** **B)** – FILTER, ALL, ALLEXCEPT, etc. are table filtering functions.  
**74.** **B)** – Default fallback controls when the engine falls back to DirectQuery.  
**75.** **B)** – Incremental refresh uses a date/time column and RangeStart/RangeEnd (or similar) parameters.

---

### 76–100. Explanations (batch)

**76.** **B)** – Viewer can view and use content but not create or edit.  
**77.** **B)** – Sensitivity labels classify and can enforce protection (encryption, access).  
**78.** **B)** – Impact analysis shows dependent items before deployment.  
**79.** **B)** – Shortcuts can reference ADLS Gen2, S3, or other OneLake locations.  
**80.** **B)** – Group By aggregates by key columns (SUM, COUNT, AVG, etc.).  
**81.** **B)** – Fact tables have foreign keys and numeric measures.  
**82.** **B)** – Visual Query Editor lets you build queries with a visual interface.  
**83.** **B)** – Windowing functions (OFFSET, INDEX, WINDOW) work over relative rows.  
**84.** **B)** – Incremental refresh reduces size and refresh time.  
**85.** **B)** – Endorsement is done by users with appropriate permissions.  
**86.** **B)** – Dataflows Gen2 can write to lakehouse, warehouse, semantic model, etc.  
**87.** **B)** – Left outer join keeps all left rows and matching right rows.  
**88.** **B)** – Composite models mix Import, DirectQuery, and/or Direct Lake.  
**89.** **B)** – Direct Lake refresh behavior is how/when the model syncs from the source.  
**90.** **B)** – Real-Time hub shows event streams, KQL databases, real-time sources.  
**91.** **B)** – Correct types ensure correct sorting, filtering, aggregation.  
**92.** **B)** – CALCULATE evaluates an expression under modified filter context.  
**93.** **B)** – Fewer visuals, filters, and simpler DAX improve performance.  
**94.** **B)** – XMLA enables programmatic read/write for Tabular Editor, DAX Studio, scripts.  
**95.** **B)** – SUMX is an example of selecting/filtering/aggregating with DAX.  
**96.** **B)** – Field parameters are created in the semantic model as a parameter table.  
**97.** **B)** – Choice depends on data type, query language, and use case.  
**98.** **B)** – Large format supports very large models with better scalability.  
**99.** **B)** – Keeping only "Status = Active" is filtering data.  
**100.** **B)** – Column-level security is implemented via roles and column visibility.

---

### 101–125. Explanations (batch)

**101.** **B)** – Member can create and edit; Admin can share and manage access.  
**102.** **B)** – Data connection = source type, connection details, authentication.  
**103.** **B)** – A view stores a SELECT as a named object.  
**104.** **B)** – Calculation groups share a common pattern across measures.  
**105.** **B)** – Direct Lake on OneLake when data is in lakehouse/OneLake-backed item.  
**106.** **B)** – T-SQL (SELECT, WHERE, GROUP BY) filters and aggregates in the warehouse.  
**107.** **B)** – .pbip contains the model definition (e.g., TMDL) for Git.  
**108.** **B)** – OneLake integration lets semantic models connect to OneLake-backed data and use Direct Lake.  
**109.** **B)** – Fill down, replace nulls, or remove rows to handle missing data.  
**110.** **B)** – VAR is evaluated once and reused.  
**111.** **B)** – File-level access via OneLake security and item permissions.  
**112.** **B)** – Ingest via pipelines, dataflows, eventstream/eventhouse.  
**113.** **B)** – Denormalizing = adding dimension attributes into a fact/wide table.  
**114.** **B)** – Dynamic format strings change format by calculation item.  
**115.** **B)** – Prefer simple aggregations and variables; avoid unnecessary iterators.  
**116.** **B)** – Visual Query Editor = select, filter, aggregate visually.  
**117.** **B)** – Pipeline rules configure what is deployed and how (e.g., parameters).  
**118.** **B)** – OneLake catalog = browse and find shared/endorsed assets.  
**119.** **B)** – Table-valued function returns a table, often with parameters.  
**120.** **B)** – Star schema improves performance and clarity.  
**121.** **B)** – Engine can fall back to DirectQuery when Direct Lake cannot satisfy the query.  
**122.** **B)** – Write permission allows editing the item.  
**123.** **B)** – Inner join keeps only matching rows.  
**124.** **B)** – Information functions support conditional logic and security.  
**125.** **B)** – Eventhouse is for high-throughput event/streaming data with KQL.

---

### 126–150. Explanations (batch)

**126.** **B)** – Storage mode determines where and how data is stored and queried.  
**127.** **B)** – Reusable assets = .pbit, .pbids, shared semantic models.  
**128.** **B)** – Identifying duplicates supports data quality and accurate analytics.  
**129.** **B)** – Partitions are based on a date/time column and ranges.  
**130.** **B)** – KQL selects, filters, aggregates in eventhouses and KQL databases.  
**131.** **B)** – Admin manages workspace (users, roles, delete, items).  
**132.** **B)** – Shortcuts are pointers; data appears without copying.  
**133.** **B)** – Coarser grain reduces rows and can improve performance.  
**134.** **B)** – Bridge table contains keys from both dimensions.  
**135.** **B)** – Limit visuals, use filters/bookmarks, optimize DAX and granularity.  
**136.** **B)** – Real-Time hub = discover and manage real-time/streaming assets.  
**137.** **B)** – Date type enables correct date filtering and time intelligence.  
**138.** **B)** – Composite = combine different sources or modes in one report.  
**139.** **B)** – Impact analysis identifies dependents to reduce breakage risk.  
**140.** **B)** – FILTER and SUM in DAX = select, filter, aggregate in the model.  
**141.** **B)** – Large format is configured in semantic model settings.  
**142.** **B)** – On-premises typically requires a gateway.  
**143.** **B)** – Adding Region from lookup = enrichment.  
**144.** **B)** – Direct Lake on SQL endpoint when source is a warehouse.  
**145.** **B)** – OLS restricts specific tables, columns, or measures.  
**146.** **B)** – Fill Down replaces nulls with the value from above.  
**147.** **B)** – Calculation groups are defined in the semantic model (Tabular Editor, etc.).  
**148.** **B)** – RangeStart/RangeEnd define the date range per partition.  
**149.** **B)** – Endorsement affects discoverability and trust in the catalog.  
**150.** **B)** – Iterators are for row-by-row or filtered-table evaluation.
